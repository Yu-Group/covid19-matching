...
from config import data_dir, viz_dir, assets_dir
import nn
import sys
...
def add_callbacks(app, df):
    ...
    def save_screenshot(selected_counties='Alameda, CA', selected_features=['recent_deaths']):
        print('selected_counties', selected_counties, 'selected_features', selected_features)
        # print(selected_features)
        # print(isinstance(selected_counties, list))
        cols = ['#e6194b', '#3cb44b', '#ffe119', '#4363d8', 
...
if __name__ == '__main__':
    ...
    def reload_and_update_app():
        ...
        redefine_app(app, df)
        add_callbacks(app, df)
        print('running matching app...last reloaded at', time.strftime("%A, %d. %B %Y %I:%M:%S %p"))
        return app


    app = reload_and_update_app()
        
    scheduler = BackgroundScheduler()
    scheduler.add_job(func=reload_and_update_app, trigger="interval", seconds=3600)
    scheduler.start()
